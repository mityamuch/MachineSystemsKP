...
**Студент:** Касьянов Илья, Муханов Дмитрий

**Группа:**  М8О-214СВ-24, М8О-209СВ-24
...
### 3.2. Transformer-архитектуры (Temporal Fusion Transformer)
...
### Уровень 1: Модель долгосрочного прогноза (TFT)
...
- **Monitoring (Prometheus & Grafana):** Мониторинг метрик системы и качества модели.
  - **Prometheus:** Собирает метрики через экспортеры и push-механизмы, визуализируют тренды дрейфа, генерируют алерты при обнаружении значительного дрейфа, триггерят переобучение модели :
  - **Grafana:** Визуализирует метрики из Prometheus и других источников:
...
3. **Автоматическая разметка данных:** Система автоматически формирует таргеты для обучения моделей:
   - Через 5 минут после момента времени `t` система извлекает фактические значения `demand(t+5)` и `supply(t+5)` из накопленных событий
   - Вычисляется таргет `demand_supply_gap(t+5) = demand(t+5) - supply(t+5)`
   - Признаки, собранные в момент `t`, автоматически связываются с таргетом `t+5`, формируя обучающие примеры
   - Разметка происходит без ручного вмешательства, что обеспечивает непрерывное обновление обучающих данных
4. **Batch Processing:** Apache Spark еженедельно обрабатывает исторические данные:
   - Формирование обучающих выборок
   - Расчёт сезонных паттернов
   - Аугментация данных:
5. **Feature Store:** Признаки сохраняются в двух слоях:
   - **Online (Redis):** Быстрые агрегаты для инференса (< 10 мс)
   - **Offline (S3 + Parquet):** Исторические данные для обучения
6. **Data Quality:** Great Expectations проверяет качество данных перед использованием.
...
## 3. Формирование признаков для обучения

**Признаки для TFT** (агрегация 5–15 мин): лаги спроса/предложения (`demand_lag_5m`, `supply_lag_1h`), пространственные (`geo_zone_id`, `poi_count`), временные (`hour_of_day`, `is_holiday`), контекстные (`temperature`, `event_nearby`).

**Признаки для LightGBM** (агрегация 1–5 мин): прогнозы TFT (`tft_gap_prediction`), streaming-агрегаты (`demand_velocity`, `driver_inflow_5m`), системные (`current_eta`, `queue_length`), дельты (`demand_delta_tft`).

**Путь данных:** Mobile Apps → Kafka → Flink (real-time) / Spark (batch) → Feature Store (Redis online + S3 offline) → Training Pipeline.

**Таргеты:** TFT: `gap(t+5) = demand[t+5] - supply[t+5]`. LightGBM: оптимальный surge на основе fill_rate и wait_time.

**Трансформации:** StandardScaler для TFT, Target Encoding для категорий, sin/cos для времени, log1p для demand/supply.

---

## 4. Архитектура Training Pipeline
...
## 5. Архитектура Inference Pipeline (Serving)
...
- **Redis Cache:** Кэширование результатов прогноза TFT модели по геозонам (TTL = 30 сек).
...
С учётом отказоустойчивости (+20%): **65 инстансов**

### Конфигурация инстансов:

| Компонент | CPU | RAM | Количество |
|-----------|-----|-----|------------|
| Inference Pod (LightGBM) | 2 vCPU | 4 ГБ | 65 |
...
- **Inference Service:** Развёрнут в Kubernetes с HPA.
...
- **Kubernetes:** Реплики каждого сервиса в разных availability zones.
